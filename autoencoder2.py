...
class AutoEncoder(tf.keras.Model):
    ...
    def encode(self, X1, X2, training=False):
        # =============================================================================
        ...
        l1 = self.l1_layer(X1)
        if not training:
            l1 = tf.keras.layers.BatchNormalization()(l1,training)
        else:
            l1 = tf.keras.layers.BatchNormalization()(l1, training=True)
        ...
        l2 = self.l2_layer(X2)
        if not training:
            l2 = tf.keras.layers.BatchNormalization()(l2, training)
        else:
            l2 = tf.keras.layers.BatchNormalization()(l2, training=True)
        ...
        if not training:
           l3 = tf.keras.layers.BatchNormalization()(l3, training)
        else:
           l3 = tf.keras.layers.BatchNormalization()(l3, training=True)
        self.n_layer3 = l3.shape
        self.l3 = l3
        print("layer3 shape", l3.shape)
        return l3

    def decode(self, H, training=False):
        self.l4_layer=tf.keras.layers.Dense(self.n_hidden1 + self.n_hidden2, kernel_initializer=self._init, name='layer4')
        l4 = self.l4_layer(H)
        if not training:
            l4 = tf.keras.layers.BatchNormalization()(l4, training)
        else:
            l4 = tf.keras.layers.BatchNormalization()(l4, training=True)
        ...
        l5 = self.l5_layer(s1)
        if not training:
            l5 = tf.keras.layers.BatchNormalization()(l5, training)
        else:
            l5 = tf.keras.layers.BatchNormalization()(l5, training=True)
        ...
        l6 = self.l6_layer(s2)
        if not training:
           l6 = tf.keras.layers.BatchNormalization()(l6, training)
        else:
           l6 = tf.keras.layers.BatchNormalization()(l6, training=True)
    ...
    def loss(self, X1, X2, training):

        self.H = self.encode(X1, X2, training)
        X1_, X2_ = self.decode(self.H, training)
        self.get_weights()
        ...
        while epoch < self.max_epochs and stop == False:
            ...
            for sample in mini_batches:
                ...
                with tf.GradientTape() as tape:
                    # 计算当前批次的损失
                    current_loss = self.loss(batch_xs1, batch_xs2,training=True)
                ...
                cost = self.loss(batch_xs1, batch_xs2,training=self.is_train)
                #print("-----------------------Kosten---------------------------",cost)
                #print("---------------------------------------------")
                avg_cost += cost[0] * len(sample) / n_samples
            ...
            for sample in mini_batches:
                ...
                cost_val = self.loss(batch_xs1, batch_xs2,training=False)
       ...
       # save_sess = self.sess
        ...
        while epoch < self.max_epochs and stop == False:
            ...
            for sample in mini_batches:
                ...
                #feed_dictio = {X1: batch_xs1, X2: batch_xs2, self.is_train: True}
