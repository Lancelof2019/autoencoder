            ...
            if avg_cost_val < best_val_cost:
              ...
              #  print("show me the costs_val_inter",costs_val_inter)
               ...
               # print("show me the costs",costs)
                ...
                costs_val_inter = []
                costs_inter = []
                #print("----------------In if teritory----------------------------")
                #print(" if costs:",costs)
                #print(" if costs_val:",costs_val)
               # print(" if best_cost:",best_cost)
               # print(" if avg_cost_val:",avg_cost_val)
                #print("----------------In if teritory ends----------------------------")
            else:
                last_improvement += 1
                costs_val += costs_val_inter  # costs history of the validation set
                costs += costs_inter
...
if __name__ == '__main__':
    ...
    f = open(
        r'C:\Users\gklizh\Documents\Workspace\code_and_data\data\python_related\result\comm_parameter_binary.txt',
        'w+')
    ...
    for iterator in range(1):
        ...
        if htseq_nbr > 1 and methy_nbr > 1:
            ...
            print("------Start trial----------------")
            best = fmin(sae.cross_validation, space, algo=tpe.suggest, max_evals=2, trials=trials)
            print("------------------Got the best value----------")
            print(best)
            print("-----------------------End output best------------")
            # save the best HPs
            fname = (r'C:\Users\gklizh\Documents\Workspace\code_and_data\data\python_related\result\comm_trials_binary_test03.pkl')
