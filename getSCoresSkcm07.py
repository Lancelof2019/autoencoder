...
class AutoEncoder(tf.keras.Model):
    ...
    def train(self):
        ...
        while epoch < self.max_epochs and stop == False:
            ...
            for sample in mini_batches:
               ...
               # cost = self.loss(batch_xs1, batch_xs2)
                cost = tf.reduce_mean(self.loss(batch_xs1, batch_xs2))
       ...
       # costfinal, res = self.sess.run([self.loss_, self.H], feed_dict=feed_dictio)
        ...
        costfinal = tf.reduce_mean(self.loss(self.training_data1, self.training_data2))
        res = self.H
        ...
        plt.savefig(r'C:\Users\gklizh\Documents\Workspace\code_and_data\figure\loss_curve\cmt_picture' + str(iterator + 1) + '.png')
        plt.close()
    ...
    def Main(self, params):
        ...
        if init == 'xavier':
            self._init = tf.keras.initializers.GlorotNormal(seed=1)
...
if __name__ == '__main__':
    ...
    selected_features = np.genfromtxt(
        r'C:\Users\gklizh\Documents\Workspace\code_and_data\data\python_related\data\selected_features.csv',
        delimiter=',',
        skip_header=1)
    # log10(htseq_fpkm + 1)
    inputhtseq = np.genfromtxt(
        r'C:\Users\gklizh\Documents\Workspace\code_and_data\data\python_related\data\exp_intgr.csv',
    ...
    inputmethy = np.genfromtxt(
        r'C:\Users\gklizh\Documents\Workspace\code_and_data\data\python_related\data\mty_intgr.csv',
        dtype=np.unicode_, delimiter=',', skip_header=1)
    ...
    namehtseq = np.genfromtxt(r'C:\Users\gklizh\Documents\Workspace\code_and_data\data\python_related\data\exp_feature_names.csv', dtype=np.unicode_, delimiter=',', skip_header=1)
    namemethy = np.genfromtxt(r'C:\Users\gklizh\Documents\Workspace\code_and_data\data\python_related\data\mty_feature_names.csv', dtype=np.unicode_, delimiter=',', skip_header=1)

    # tanh activation function
    act = tf.nn.tanh

    #fname = './result/32run/comm_trials_binary.pkl'
    #fname = (r'C:\Users\gklizh\Documents\Workspace\code_and_data\data\python_related\result\comm_trials_binary_test.pkl')
    fname = (r'C:\Users\gklizh\Documents\Workspace\code_and_data\data\python_related\result\comm_trials_binary_test11.pkl')
        ...
        if htseq_nbr > 1 and methy_nbr > 1:
           ...
           # loss, h, shapfeat = sae.Main(best)
            ...
            if loss < best_loss:
                ...
                #with open(r'C:\Users\gklizh\Documents\Workspace\code_and_data\data\python_related\shap_doc\\' + cmtname + 'shapfeat_htseq.csv', 'w') as csvfile:
                   # writer = csv.writer(csvfile)
                   # [writer.writerow(r) for r in np.vstack((namehtseq_sel.reshape(1, len(namehtseq_sel)),
                                                          # shapfeat[0][0]))]

                #with open(r'C:\Users\gklizh\Documents\Workspace\code_and_data\data\python_related\shap_doc\\' + cmtname + 'shapfeat_methy.csv', 'w') as csvfile:
                   # writer = csv.writer(csvfile)
                ...
                #fname = r'C:\Users\gklizh\Documents\Workspace\code_and_data\data\python_related\shap_doc\cmt_SHAPValues.pkl'
                #pickle.dump(shapfeat, open(fname, "wb"))
                with open(r'C:\Users\gklizh\Documents\Workspace\code_and_data\data\python_related\result\community\communityScores_compare.csv', 'w') as csvfile:
                    writer = csv.writer(csvfile)
                    [writer.writerow(r) for r in cmt_scores]
                #with open(r'C:\Users\gklizh\Documents\Workspace\code_and_data\data\python_related\shap_doc\\' + cmtname + 'reshtseq.csv', 'w') as csvfile:
                   # writer = csv.writer(csvfile)
                    #[writer.writerow(r) for r in reshtseq]
                #with open(r'C:\Users\gklizh\Documents\Workspace\code_and_data\data\python_related\shap_doc\\' + cmtname + 'resmethy.csv', 'w') as csvfile:
                    #writer = csv.writer(csvfile)
